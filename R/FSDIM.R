...
.EMVS <- function(M, G, grouping,
                  ...
                  thresh=0.0001,
                  transform_M="L"){
  #### NEED TO FIGURE OUT A WAY TO AVOID VARIABLE SHADOWING
  ...
  } else if(transform_M == "Q"){
    M <- cbind(M, M^2)

  } else if(transform_M == "QS"){
    M <- apply(M, 2, function(x){
      x <- splines::ns(x)
      attributes(x) <- NULL
      x
    })
  } else stop("Invalid value for transform_M. Choose from 'L', 'Q', 'QS'")
...
#'
#' @export
Zmat_builder <- function(R2, G, lower_R2=0.2, upper_R2=0.8){
  K <- ncol(G)
  Zmatrix <- cbind(matrix(1,nrow=K,ncol=1), matrix(0,nrow=K,ncol=3))

  # hao
  for(ww in 1:K){
    if(R2[ww] > upper_R2) Zmatrix[ww, 2] <- 1
    else{
      if(R2[ww] >= lower_R2) Zmatrix[ww, 3] <- 1
      else Zmatrix[ww, 4] <- 1
    }
  }

  rownames(Zmatrix) <- colnames(G)
  colnames(Zmatrix) <- c("General", "M", "M + M^C", "M^C") # check if this is true
...
#'
#' @export
FSDIM <- function(M, G, grouping, nu0=0.5, nu1=10^3, nu=1, lambda=1, a=1, b=1,
                  I=100,
                  thresh=0.0001,
                  lower_R2 = 0.2,
                  upper_R2 = 0.8,
